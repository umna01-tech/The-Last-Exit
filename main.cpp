...
int main() {
    ...
    int playerMoney = 0;
    Difficulty difficultyScreen(font);
    ...
    sf::Vector2f mousePos; // ← declare here so whole loop can see it!
    ...
    while (window.isOpen()) {
        ...
        while (auto event = window.pollEvent()) {
            ...
            if (event->is<sf::Event::MouseButtonPressed>()) {
                ...
                mousePos = sf::Vector2f(mouse); // ← update it here
                ...
                if (currentScreen == Screen::Difficulty) {
                    if (difficultyScreen.handleClick(mousePos, playerMoney)) {
                        currentScreen = Screen::Difficulty; // stay on difficulty
                    }
                }
            }
        }
        ...
        if (currentScreen == Screen::MainMenu) {
            ...
            difficultyScreen.draw(window);
        }

        window.display(); // ← inside the loop! ✅
    }
